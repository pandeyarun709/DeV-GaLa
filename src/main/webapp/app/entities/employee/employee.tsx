  ...
  return (
    <div>
      <h2 id="employee-heading" data-cy="EmployeeHeading">
        <Translate contentKey="hospitalBeApp.employee.home.title">Employees</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="hospitalBeApp.employee.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/employee/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="hospitalBeApp.employee.home.createLabel">Create new Employee</Translate>
          </Link>
        </div>
      </h2>
          ...
          {employeeList && employeeList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="hospitalBeApp.employee.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  ...
                  <th className="hand" onClick={sort('rmoHourlyRate')}>
                    <Translate contentKey="hospitalBeApp.employee.rmoHourlyRate">Rmo Hourly Rate</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('rmoHourlyRate')} />
                  </th>
                  ...
                  <th>
                    <Translate contentKey="hospitalBeApp.employee.department">Department</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="hospitalBeApp.employee.test">Test</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {employeeList.map((employee, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{employee.registrationNo}</td>
                    <td>{employee.rmoHourlyRate}</td>
                    <td>
                      <Translate contentKey={`hospitalBeApp.EmployeeType.${employee.type}`} />
                    </td>
                    <td>{employee.isActive ? 'true' : 'false'}</td>
                    <td>{employee.createdBy}</td>
                    <td>{employee.createdOn ? <TextFormat type="date" value={employee.createdOn} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{employee.updatedBy}</td>
                    <td>{employee.updatedOn ? <TextFormat type="date" value={employee.updatedOn} format={APP_DATE_FORMAT} /> : null}</td>
                    <td>{employee.department ? <Link to={`/department/${employee.department.id}`}>{employee.department.id}</Link> : ''}</td>
                    <td>{employee.test ? <Link to={`/diagnostic-test/${employee.test.id}`}>{employee.test.id}</Link> : ''}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="hospitalBeApp.employee.home.notFound">No Employees found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
